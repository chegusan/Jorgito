"""Phase 4: render all 9 atlas states at REAL terminal size and combine them
into one labeled contact sheet -- the final visual gate artifact.
...
``script``-faked TTYs (see ``terminal_render_phase4/*.script.txt``, produced
separately) -- this script turns that same pixel data into an inspectable PNG,
same pattern as Phase 1's ``render_phase1_test_pet.py``.
...
OUT_DIR = REPO / "assets/keyframes/terminal_render_phase4"
...
def main() -> None:
    ...
    sheet_path = OUT_DIR / "contact_sheet_terminal_phase4.png"
